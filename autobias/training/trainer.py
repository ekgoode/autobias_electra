...
class Trainer(Configured):
  ...
  def training_run(
      ...
      # Handle None for eval_sets
      eval_sets = self.eval_sets if self.eval_sets is not None else []

      if self.evaluator is not None and eval_sets:
      ...
      examples = model.preprocess_train(
          train_dataset, [x.dataset for x in eval_sets] if eval_sets else [], n_processes
      )
      ...
      if eval_sets and self.evaluator:
